...
export default function HomeVideo({
  ...
  async function increaseView() {
    fetch(
      `http://localhost:4000/videosViews/${
        video
          ? video.id
          : videoLiked
          ? videoLiked.id
      ...
      }`,
      {
        ...
        body: JSON.stringify({
          views: video
            ? video.views + 1
            : videoLiked
            ? videoLiked?.video?.views + 1
            : videoSaved
            ? videoSaved?.video?.views + 1
            : videoMine
            ? videoMine?.video?.views + 1
            : null,
        }),
      }
    )
  ...
  async function removeVideoSaved(videoId: any, userId: any) {
    const videosSavedArray = videosSaved?.find(
      (videoSaved: any) =>
        videoSaved?.userId === userId && videoSaved?.videoId === videoId
    );
      ...
      video ? (
        <>
          ...
          >
            ...
            <img
              className="icon-post"
              src={`http://localhost:4000/avatar/${video?.userWhoCreatedIt?.userName}`}
              alt=""
            />
        ...
        <>
          ...
          >
            <img
              className="image-post"
              src={`http://localhost:4000/thumbnail/${videoLiked?.video?.title}`}
              alt=""
            />
            <h2 className="video-title">{videoLiked?.video?.title}</h2>
            <span className="video-views">
              {videoLiked?.video?.views} views - {videoLiked?.video?.createdAt}{" "}
            </span>
          </div>
        </>
      ) : liked === "not" &&
        ...
        <>
          ...
          >
            <img
              className="image-post"
              src={`http://localhost:4000/thumbnail/${videoSaved?.video?.title}`}
              alt=""
            />
            <img
              className="icon-post"
              src={`http://localhost:4000/avatar/${videoSaved?.video?.userWhoCreatedIt?.userName}`}
              alt=""
            />
            ...
            >
              {videoSaved?.video?.userWhoCreatedIt?.userName}
            </span>
            <span className="video-views">
              {videoSaved?.video?.views} views - {videoSaved?.video?.createdAt}{" "}
            </span>
            <button
              className="remove-video-saved"
              onClick={function (e) {
                e.stopPropagation();
        ...
        <>
          ...
          >
            <img
              className="image-post"
              src={`http://localhost:4000/thumbnail/${videoMine?.title}`}
              alt=""
            />
            <img
              className="icon-post"
            ...
            <span className="video-views">
              {videoMine?.views} views - {videoMine?.createdAt}
            </span>
            <button
              className="remove-my-video"
              onClick={function (e) {
                e.stopPropagation();
